...
double producer() {
	...
	for(size_t i = 0; i < total_length; i++) {
		...
		if(vec.size() % chunk_size == 0) {
			std::cout << std::format("Sending chunk data to rank = {}\n",
			                         rank_to_send);
			MPI_Request request;
			MPI_Isend(vec.data() + starting_idx, chunk_size, MPI_DOUBLE,
			          rank_to_send, 0, MPI_COMM_WORLD, &request);
			requests.emplace_back(std::move(request));
			starting_idx += chunk_size;
			rank_to_send
				= ((rank_to_send + 1) % mpi_size) == 0 ? 1 : (rank_to_send + 1);
		}
	}
	...
	for(int rank = 1; rank < mpi_size; rank++) {
		...
		MPI_Recv(&local_sum, 1, MPI_DOUBLE, rank, /* tag = */ 2, MPI_COMM_WORLD,
		         nullptr);
		sum += local_sum;
	}
	return sum;
...
void consumer() {
	...
	while(true) {
		...
		if(status.MPI_TAG == 1) {
			...
			MPI_Recv(&op, 1, MPI_UNSIGNED, /* source = */ 0, /* tag = */ 1,
			...
			MPI_Recv(buf.data(), chunk_size, MPI_DOUBLE, /* source = */ 0,
			         /* tag = */ 0, MPI_COMM_WORLD, nullptr);
	...
	std::cout << std::format("Sending local summation, mpi_rank = {}\n",
	                         mpi_rank);
	MPI_Send(&local_sum, 1, MPI_DOUBLE, 0, /* tag = */ 2,
	         MPI_COMM_WORLD); // use tag == 2 to send the final result
}
